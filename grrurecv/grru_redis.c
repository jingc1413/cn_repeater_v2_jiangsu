...
RESULT RedisHsetAndPush(PXMLSTRU  pstruXml, int nType)
{
    ...
    if (nType==1)
    { 
    	reply = redisCommand(redisconn,"LPUSH %u_%d %s", atol(DemandStrInXmlExt(pstruXml, "<omc>/<站点编号>")),
    		atoi(DemandStrInXmlExt(pstruXml, "<omc>/<设备编号>")),  pMessageBoby);
    	PrintDebugLog(DBG_HERE, "LPUSH %u_%d %s", atol(DemandStrInXmlExt(pstruXml, "<omc>/<站点编号>")),
    		atoi(DemandStrInXmlExt(pstruXml, "<omc>/<设备编号>")),  pMessageBoby);
    }
    else
    { 
